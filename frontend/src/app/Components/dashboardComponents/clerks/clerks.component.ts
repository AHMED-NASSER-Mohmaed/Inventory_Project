...
export class ClerksComponent implements OnInit, OnDestroy{
  ...
  // Filter state and pagination
        ...
        activateAdminAssignment(): void {
          ...
          const sub = this.clerkService.activateCustomerWithBranch(this.selectedActivationAdmin._id, this.selectedActivationBranch)
            .subscribe({
              next: (res) => {
        ...
        toggleEdit(event?: any): void {
          if (this.editing) {
            ...
            const sub = this.clerkService.updateCustomer(this.selectedUser._id, updatePayload)
              .subscribe({
                next: (res: any) => {
