...
export class ClerksComponent implements OnInit, OnDestroy{
  ...
  currentFilter: string = '';
        ...
        loadSellers(): void {
          ...
          const obs = this.clerksService.getPaginatedCustomersByStatus(
            this.currentPage,
            this.itemsPerPage,
            filterParam,
            sortParam
        ...
        deActiveCustomer(_id: string): void {
          const sub = this.clerksService.deActiveCustomer(_id).subscribe({
            next: (res) => {
              console.log(res);
              this.pageCache = {};
              if(this.isSearchMode) {
        ...
        activateCustomer(_id: string): void {
          const sub = this.clerksService.activateCustomer(_id).subscribe({
            next: (res) => {
              console.log(res);
              this.pageCache = {};
              if(this.isSearchMode) {
        ...
        activateAdminAssignment(): void {
          ...
          const sub = this.clerksService.activateCustomerWithBranch(this.selectedActivationAdmin._id, this.selectedActivationBranch)
            .subscribe({
              next: (res) => {
        ...
        getActiveCustomersCount(): void {
          const sub = this.clerksService.getActiveCustomersCount().subscribe({
        ...
        getInActiveCustomersCount(): void {
          const sub = this.clerksService.getInActiveCustomersCount().subscribe({
        ...
        toggleEdit(event?: any): void {
          if (this.editing) {
            ...
            const sub = this.clerksService.updateCustomer(this.selectedUser._id, updatePayload)
              .subscribe({
                next: (res: any) => {
          ...
          if (file && this.selectedUser) {
            ...
            reader.onload = (e: any) => {
              ...
              const sub = dialogRef.afterClosed().subscribe(async (result) => {
                if (result) {
                  try {
                    const response: any = await this.clerksService
        ...
        addAdmin(): void {
          ...
          const sub = this.clerksService.addAdmin(payload).subscribe({
            next: (res) => {
        ...
        loadBranches(): void {
          this.clerksService.getMappedBranches().subscribe({
            next: (res) => {
