    ...
    const sub = this.customerService.deActiveCustomer(_id).subscribe({
      next: (res) => {
        ...
        if (deactivatedSeller) {
          ...
          const activeKey = `active_${this.currentPage}_${this.sortField}_${this.sortDirection}_isActive:true`;
          if (this.pageCache[activeKey]) {
            this.pageCache[activeKey].result = this.pageCache[
              activeKey
            ].result.filter((user) => user._id !== _id);
            this.pageCache[activeKey].total--;
          }

          // Add to first page of inactive cache
          const inactiveKey = `active_1_${this.sortField}_${this.sortDirection}_isActive:false`;
    ...
    const sub = this.customerService.activateCustomer(_id).subscribe({
      next: (res) => {
        ...
        if (activatedSeller) {
          ...
          const inactiveKey = `active_${this.currentPage}_${this.sortField}_${this.sortDirection}_isActive:false`;
          if (this.pageCache[inactiveKey]) {
            this.pageCache[inactiveKey].result = this.pageCache[
              inactiveKey
            ].result.filter((user) => user._id !== _id);
          ...
          const activeKey = `active_1_${this.sortField}_${this.sortDirection}_isActive:true`;
          if (this.pageCache[activeKey]) {
