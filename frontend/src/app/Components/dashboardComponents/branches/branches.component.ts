...
export class BranchesComponent {
  ...
  // Filter state and pagination
          ...
          loadSellers(): void {
            ...
            const obs = this.branchService.getPaginatedCustomersByStatus(
              this.currentPage,
              this.itemsPerPage,
              filterParam,
              sortParam
          ...
          deActiveCustomer(_id: string): void {
            const sub = this.branchService.deActiveCustomer(_id).subscribe({
              next: (res) => {
                console.log(res);
          ...
          activateCustomer(_id: string): void {
            const sub = this.branchService.activateCustomer(_id).subscribe({
              next: (res) => {
                console.log(res);
                this.pageCache = {};
                if(this.isSearchMode) {
          ...
          activateAdminAssignment(): void {
            ...
            const sub = this.branchService.activateCustomerWithBranch(this.selectedActivationAdmin._id, this.selectedActivationBranch)
              .subscribe({
                next: (res) => {
          ...
          getActiveCustomersCount(): void {
            const sub = this.branchService.getActiveCustomersCount().subscribe({
          ...
          getInActiveCustomersCount(): void {
            const sub = this.branchService.getInActiveCustomersCount().subscribe({
          ...
          toggleEdit(event?: any): void {
            if (this.editing) {
              ...
              const sub = this.branchService.updateCustomer(this.selectedUser._id, updatePayload)
                .subscribe({
                  next: (res: any) => {
            ...
            if (file && this.selectedUser) {
              ...
              reader.onload = (e: any) => {
                ...
                const sub = dialogRef.afterClosed().subscribe(async (result) => {
                  if (result) {
                    try {
                      const response: any = await this.branchService
          ...
          addAdmin(): void {
            ...
            const sub = this.branchService.addAdmin(payload).subscribe({
              next: (res) => {
          ...
          loadBranches(): void {
            this.branchService.getMappedBranches().subscribe({
              next: (res) => {
