...
class OrderContainerController {
  ...
  initializeRoutes() {
    ...
    this.router.get(
      ...
      catchAsync(this.getSubOrdersForCustomerByStatus)
    );
    // i need here order id as a route parameter as you see below
    this.router.patch(
      "/customerOrders/cancelWholeOrder/:orderId",
      AuthMiddleware.protect,
      catchAsync(this.cancelSubOrderForCustomer)
    );
    ...
    this.router.patch(
      "/customerOrders/cancelSomeProductsInTheOrder/:orderId",
      AuthMiddleware.protect,
      catchAsync(this.cancelSubOrderWithProductIdsForCustomer)
    );
  ...
  async getOrderContainerById(req, res) {
    const orderContainer = await OrderContainerService.getOrderContainerById(
      req.params.id
    );
  ...
  async cashierFinalisOnlineOrderByCompleteStatus(req, res) {
    if (req.user.role == "cashier") {
      // becasue  admin can process the order too but it has to be admin of that branch
      ...
      if (req.user.branch != APP_CONFIG.ONLINE_BRANCH_ID) {
        throw new AppError(
          "You are not authorized to process this order since you are not employed in this branch."
        );
      }
    }
    ...
    const subOrder =
      await SubOrderService.cashierFinalisOnlineOrderByCompleteStatus({
        orderId,
        cashierId,
      });
  ...
  async getAllOnlineOrdersForCashier(req, res) {
    ...
    const subOrders =
      await SubOrderService.getAllOnlineOrdersForCashierBasedOnStatus(
        cashierId,
        status
      );
    res.status(APP_CONFIG.HTTP_OK).json({
  ...
  async processOnlineOrderForClerkOrExternalSeller(req, res) {
    ...
    const subOrder =
      await SubOrderService.processOnlineOrderForClerkOrExternalSeller(
        req.body
      );
    res.status(APP_CONFIG.HTTP_OK).json({
      message: "success",
  ...
  async createOfflineOrderContainer(req, res) {
    ...
    const orderContainer =
      await OrderContainerService.createOfflineOrderContainer(req.body);
    res.status(APP_CONFIG.HTTP_CREATED).json({
      message: "success",
      orderContainer,
    });
  ...
  async finalizeOfflineOrderContainer(req, res) {
    ...
    const orderContainer =
      await OrderContainerService.finalizeOfflineOrderContainerForCashier(data);
    res.status(APP_CONFIG.HTTP_OK).json({
      message: "success",
      orderContainer,
    });
  ...
  async getOfflineOrderContainers(req, res) {
    ...
    const orderContainers =
      await OrderContainerService.getOrderOfflineContainers(
        status,
        req.user.branch
      );
    res.status(APP_CONFIG.HTTP_OK).json({
  ...
  async getAllOfflineSubordersForSuperAdmin(req, res) {
    const allOfflineSuborders =
      await orderService.getAllOfflineSubOrdersForSuperAdmin();
    res.status(APP_CONFIG.HTTP_OK).json({
      message: "success",
      allOfflineSuborders,
    });
  ...
  async getAllOnlineSubordersForSuperAdmin(req, res) {
    const allOnlineSuborders =
      await orderService.getAllOnlineSubOrdersForSuperAdmin();
    res.status(APP_CONFIG.HTTP_OK).json({
      message: "success",
      allOnlineSuborders,
    });
  ...
  async cancelSubOrderForCustomer(req, res) {
    ...
    const customerId = req.user.id;
    const response = await orderService.cancelSubOrderByCustomer(
      customerId,
      orderId
    );
  ...
  async cancelSubOrderWithProductIdsForCustomer(req, res) {
    ...
    const response =
      await orderService.cancelOnlineProductsFromSubOrderByCustomer(
        customerId,
        orderId,
        productIds
      );
  ...
  async getSubOrdersForCustomerByStatus(req, res) {
    ...
    const orders = await orderService.getAllOnlineSubOrdersForCustomerByStatus(
      customerId,
      status
    );
